...
The core idea is a graph. Each image is a node; the task is to group nodes that depict the same individual. Neural networks serve as similarity engines: their embeddings and local match statistics become edge weights. When the similarity between two images crosses a species threshold, an edge is drawn; connected components are the clusters. Two signals feed those weights: a global descriptor over whole-image appearance and a local keypoint matcher anchoring fine-scale marks. Which signal dominates depends on the species. Clustering then reduces to a threshold decision: too tight and known individuals fracture; too loose and strangers merge. Both the signal weights and thresholds are chosen per species — that is most of the design.
