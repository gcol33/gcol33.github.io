...
Lecture 7 continued the discussion on random walks by integrating movement simulations with spatially explicit raster grids. Random-walk processes can be implemented efficiently on discrete raster environments. Working with rasters involves defining a spatially explicit grid characterized by:
...
When simulating dispersal on a raster, the movement rules had to be adapted. Only the final position of a random walk mattered for population updates. Thus, the original random walk function was adjusted to return only final displacement coordinates, significantly enhancing computational efficiency. This was particularly important when simulating dispersal for large numbers of individuals over multiple time steps. By overlaying dispersal models onto a raster, movement becomes discrete, meaning that individuals transition from one cell to another rather than moving freely in continuous space.

#### Random Walks on a Raster Grid

Only the final displacement of an individual matters for updating population distributions. This significantly improves computational efficiency, especially when simulating dispersal across large landscapes. To determine dispersal:
