"""Render Liam's workshop HTML pages to PDF with cover, navigable TOC, and bookmarks."""
...
sections = {
    "en": [
        ...
        {"day": "Day 1", "part": "Exercises", "subtitle": "R Basics to First Data Analysis", "file": "day1_exercises.html"},
        {"day": "Day 2", "part": "Theory", "subtitle": "Algorithms & Spatial Ordering", "file": "day2_theory.html"},
        {"day": "Day 2", "part": "Exercises", "subtitle": "A Mini Research Project", "file": "day2_exercises.html"},
    ...
    "de": [
        ...
        {"day": "Tag 1", "part": "Übungen", "subtitle": "R-Grundlagen bis zur ersten Datenanalyse", "file": "day1_exercises.html"},
        {"day": "Tag 2", "part": "Theorie", "subtitle": "Algorithmen & räumliche Ordnung", "file": "day2_theory.html"},
        {"day": "Tag 2", "part": "Übungen", "subtitle": "Ein Mini-Forschungsprojekt", "file": "day2_exercises.html"},
...
def make_toc_html(lang, secs, page_numbers):
    """Page 2: navigable table of contents with page numbers."""
    ci = cover_info[lang]
...
def render_file_to_pdf(browser, html_path, out_path, hide_cls, show_cls, lang_suffix):
    """Render an HTML file to PDF with language toggling."""
    page = browser.new_page()
    ...
    page.evaluate(f"""() => {{
        ...
        // Fix inline code font size (source HTML sets it to 11px)
        // Fix images overflowing page width
...
with sync_playwright() as p:
    ...
    for lang_suffix, hide_cls, show_cls, lang_label in languages:
        ...
        # --- Pass 1: Render cover + all dividers/content to get page counts ---
        print("  Rendering cover...")
        cover_tmp = docs_dir / f"_temp_{lang_suffix}_cover.pdf"
        render_html_to_pdf(browser, make_cover_html(lang_suffix), cover_tmp)
        temp_files.append(cover_tmp)

        # Render each section (divider + content), track page numbers
        # Page 1 = cover, page 2 = TOC (to be inserted), sections start at page 3
        section_pdfs = []  # list of (divider_path, content_path)
        current_page = 3   # after cover (1) + toc (2)
        section_start_pages = []
        ...
        for sec in secs:
            ...
            div_tmp = docs_dir / f"_temp_{lang_suffix}_div_{html_file.replace('.html', '.pdf')}"
            render_html_to_pdf(browser, make_divider_html(sec), div_tmp)

            # Content
            print(f"  Rendering {html_file}...")
            content_tmp = docs_dir / f"_temp_{lang_suffix}_{html_file.replace('.html', '.pdf')}"
            render_file_to_pdf(browser, html_path, content_tmp, hide_cls, show_cls, lang_suffix)

            # Record the divider page number as this section's start
            section_start_pages.append(current_page)

            # Count pages in divider + content
        ...
        # --- Stamp page numbers (skip cover + toc = first 2 pages) ---
        ...
        day_groups = {}  # day_label -> [(part, page_idx)]
        page_idx = 2  # after cover + toc
        for sec, (div_path, content_path) in zip(secs, section_pdfs):
            ...
            day_groups[day_label].append((sub_label, page_idx))

            div_pages = len(PdfReader(str(div_path)).pages)
            content_pages = len(PdfReader(str(content_path)).pages)
            page_idx += div_pages + content_pages
        ...
        # Write final PDF
        output_path = output_dir / f"liams_workshop_{lang_suffix}.pdf"
